...
if [[ "${PHP_OPCACHE_ENABLE}" =~ ^(true|on|1)$ ]]; then
    sed -i "s|{{PHP_OPCACHE_ENABLE}}|${PHP_OPCACHE_ENABLE}|g" $PHP_INI_DIR/conf.d/php.ini
    echo "OPCache is enabled"
else
    sed -i "s|{{PHP_OPCACHE_ENABLE}}|0|g" $PHP_INI_DIR/conf.d/php.ini
    echo "OPCache is disabled"
fi

if [[ "${PHP_OPCACHE_CONSISTENCY_CHECKS}" =~ ^(true|on|1)$ ]]; then
    sed -i "s|{{PHP_OPCACHE_CONSISTENCY_CHECKS}}|${PHP_OPCACHE_CONSISTENCY_CHECKS}|g" $PHP_INI_DIR/conf.d/php.ini
    echo "OPCache Consistency Checks is enabled"
else
    sed -i "s|{{PHP_OPCACHE_CONSISTENCY_CHECKS}}|0|g" $PHP_INI_DIR/conf.d/php.ini
    echo "OPCache Consistency Checks is disabled"
fi

if [[ "${PHP_OPCACHE_VALIDATE_TIMESTAMPS}" =~ ^(true|on|1)$ ]]; then
    sed -i "s|{{PHP_OPCACHE_VALIDATE_TIMESTAMPS}}|${PHP_OPCACHE_VALIDATE_TIMESTAMPS}|g" $PHP_INI_DIR/conf.d/php.ini
    echo "OPCache Validate Timestamps is enabled"
else
    sed -i "s|{{PHP_OPCACHE_VALIDATE_TIMESTAMPS}}|0|g" $PHP_INI_DIR/conf.d/php.ini
    echo "OPCache Validate Timestamps is disabled"
fi

sed -i "s|{{PHP_OPCACHE_MEMORY_CONSUMPTION}}|${PHP_OPCACHE_MEMORY_CONSUMPTION}|g" $PHP_INI_DIR/conf.d/php.ini
sed -i "s|{{PHP_OPCACHE_MAX_ACCELERATED_FILES}}|${PHP_OPCACHE_MAX_ACCELERATED_FILES}|g" $PHP_INI_DIR/conf.d/php.ini

sed -i "s|{{PHP_XDEBUG_MODE}}|${PHP_XDEBUG_MODE}|g" $PHP_INI_DIR/conf.d/php.ini
sed -i "s|{{PHP_XDEBUG_REMOTE_HOST}}|${PHP_XDEBUG_REMOTE_HOST}|g" $PHP_INI_DIR/conf.d/php.ini
sed -i "s|{{PHP_XDEBUG_REMOTE_PORT}}|${PHP_XDEBUG_REMOTE_PORT}|g" $PHP_INI_DIR/conf.d/php.ini
sed -i "s|{{PHP_XDEBUG_IDEKEY}}|${PHP_XDEBUG_IDEKEY}|g" $PHP_INI_DIR/conf.d/php.ini
